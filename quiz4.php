  ...
  $SQL = "SELECT * FROM users WHERE uname = '".$_SESSION['uname']."'";//select statement variable that stores conditions for a query
   ...
   if($q4Done == 0){
	   if (isset($_POST["submit"]))  {
		$q4Done = 1;
		$progress += 1;
		$q4Done = mysqli_real_escape_string($conn, $q4Done);
		$q4g = mysqli_real_escape_string($conn,$q4g);
		...
		$q4g++;
		  ...
		  if(strcmp($_POST["q5"], $ra5) == 0){
			...
			$query = "UPDATE `gradebook` SET `q4Done`='$q4Done',`q4g`='$q4g',`progress`='$progress' WHERE `userID` = '$userID'";
